				...
				if (Result.Split(TEXT("#"), &DepotPathName, &ExtraInfoAfterPound))
				{
					if (DepotPathName.EndsWith(TEXT(".uasset")) || DepotPathName.EndsWith(TEXT(".umap")))
					{
						FString FullPackageName;
						{
							...
							if (DepotPathName.Split(TEXT("YQZY/Content/"), nullptr, &PostContentPath)) //@TODO: RENAME: Potential issue when modules are renamed
							{
						...
						if (FullPackageName.IsEmpty())
						{
							...
							if (DepotPathName.Split(TEXT("YQZY/Plugins/"), nullptr, &PostPluginsPath))
							{
					...
					{
						FString PostYQZYGamePath;
						if (DepotPathName.Split(TEXT("/YQZY/"), nullptr, &PostYQZYGamePath))
						{
							if (DepotPathName.EndsWith(TEXT(".cpp")))
							{
								OutChangedCode.Add(PostYQZYGamePath);
							}
							...
							{
								OutChangedOtherFiles.Add(PostYQZYGamePath);
							}
						}
					}
				}
