...
#include "YQZYPerformanceSettings.generated.h"
...
USTRUCT()
struct FYQZYQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FYQZYPerformanceStatGroup
{
	GENERATED_BODY()
...
class UYQZYPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UYQZYPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UYQZYPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FYQZYQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EYQZYFramePacingMode FramePacingMode = EYQZYFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by YQZY.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EYQZYFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="YQZY Performance Settings"))
class UYQZYPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UYQZYPerformanceSettings();
...
public:
	...
	TArray<FYQZYPerformanceStatGroup> UserFacingPerformanceStats;
};
