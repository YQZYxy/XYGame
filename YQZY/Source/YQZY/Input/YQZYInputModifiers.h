...
#include "YQZYInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UYQZYAimSensitivityData;
class UObject;
...
class UYQZYSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the YQZY Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "YQZY Settings Driven Dead Zone"))
class UYQZYInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EYQZYTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in YQZY Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "YQZY Gamepad Sensitivity"))
class UYQZYInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EYQZYTargetingType TargetingType = EYQZYTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UYQZYAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the YQZY Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "YQZY Aim Inversion Setting"))
class UYQZYInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
