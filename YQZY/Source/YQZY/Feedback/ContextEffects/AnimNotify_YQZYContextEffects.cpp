...
#include "AnimNotify_YQZYContextEffects.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_YQZYContextEffects)



UAnimNotify_YQZYContextEffects::UAnimNotify_YQZYContextEffects()
{
}

void UAnimNotify_YQZYContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_YQZYContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_YQZYContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_YQZYContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> YQZYContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UYQZYContextEffectsInterface>())
			{
				// If so, add it to the Array
				YQZYContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UYQZYContextEffectsInterface>())
					{
						YQZYContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* YQZYContextEffectImplementingObject : YQZYContextEffectImplementingObjects)
			{
				if (YQZYContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IYQZYContextEffectsInterface::Execute_AnimMotionEffect(YQZYContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UYQZYContextEffectLibrary type
						if (UYQZYContextEffectsLibrary* EffectLibrary = Cast<UYQZYContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_YQZYContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_YQZYContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
