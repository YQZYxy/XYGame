...
#include "CustomSettings/YQZYSettingAction_SafeZoneEditor.h"
#include "CustomSettings/YQZYSettingValueDiscrete_MobileFPSType.h"
#include "CustomSettings/YQZYSettingValueDiscrete_OverallQuality.h"
#include "CustomSettings/YQZYSettingValueDiscrete_Resolution.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(EYQZYFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const EYQZYFramePacingMode ActualMode = UYQZYPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	EYQZYFramePacingMode DesiredMode;
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UYQZYPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UYQZYLocalPlayer* YQZYLocalPlayer = CastChecked<UYQZYLocalPlayer>(LocalPlayer);
		YQZYLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UYQZYGameSettingRegistry::InitializeVideoSettings(UYQZYLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			UYQZYSettingValueDiscrete_Resolution* Setting = NewObject<UYQZYSettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UYQZYSettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UYQZYSettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			UYQZYSettingAction_SafeZoneEditor* Setting = NewObject<UYQZYSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FYQZYQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			UYQZYSettingValueDiscrete_MobileFPSType* Setting = NewObject<UYQZYSettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					UYQZYSettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const UYQZYLocalPlayer* YQZYLocalPlayer = CastChecked<UYQZYLocalPlayer>(LocalPlayer);
					...
					YQZYLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const UYQZYPlatformSpecificRenderingSettings* PlatformSettings = UYQZYPlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == EYQZYFramePacingMode::MobileStyle);

				const UYQZYLocalPlayer* YQZYLocalPlayer = CastChecked<UYQZYLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = YQZYLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UYQZYSettingValueDiscrete_OverallQuality* Setting = NewObject<UYQZYSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UYQZYPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UYQZYGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UYQZYLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UYQZYSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UYQZYSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UYQZYSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UYQZYSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EYQZYFramePacingMode::DesktopStyle));
