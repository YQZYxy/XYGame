...
#include "YQZYDamageLogDebuggerComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYDamageLogDebuggerComponent)

UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_YQZY_Damage_Message);

UYQZYDamageLogDebuggerComponent::UYQZYDamageLogDebuggerComponent(const FObjectInitializer& ObjectInitializer)
...
void UYQZYDamageLogDebuggerComponent::BeginPlay()
{
	Super::BeginPlay();

	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
	ListenerHandle = MessageSubsystem.RegisterListener(TAG_YQZY_Damage_Message, this, &ThisClass::OnDamageMessage);
}

void UYQZYDamageLogDebuggerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
...
void UYQZYDamageLogDebuggerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if ((TimeSinceDamage >= SecondsBetweenDamageBeforeLogging) && (DamageLog.Num() > 0))
	{
		...
		UE_LOG(YQZYLog, Warning, TEXT("%d impacts in %d distinct frames over %.2f seconds did %.2f damage"),
			NumImpacts, NumFrames, TotalInterval, TotalDamage);
		if (TotalInterval > 0.0)
		{
			UE_LOG(YQZYLog, Warning, TEXT("Interval ranged from %.1f ms to %.1f ms (avg %.1f ms)"),
				MinInterval * 1000.0, MaxInterval * 1000.0, (MaxInterval + MinInterval) / 2.0 * 1000.0);
			UE_LOG(YQZYLog, Warning, TEXT("DPS %.2f"), TotalDamage / TotalInterval);
		}
		UE_LOG(YQZYLog, Warning, TEXT("\n"));
	}
}

void UYQZYDamageLogDebuggerComponent::OnDamageMessage(FGameplayTag Channel, const FYQZYVerbMessage& Payload)
{
