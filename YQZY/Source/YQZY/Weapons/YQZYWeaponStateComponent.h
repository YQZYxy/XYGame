...
#include "YQZYWeaponStateComponent.generated.h"
...
struct FYQZYScreenSpaceHitLocation
{
...
struct FYQZYServerSideHitMarkerBatch
{
	FYQZYServerSideHitMarkerBatch() { }

	FYQZYServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FYQZYScreenSpaceHitLocation> Markers;
...
UCLASS()
class UYQZYWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UYQZYWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FYQZYScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FYQZYScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FYQZYServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
