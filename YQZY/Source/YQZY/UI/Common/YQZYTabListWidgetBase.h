...
#include "YQZYTabListWidgetBase.generated.h"
...
class UYQZYTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IYQZYTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FYQZYTabDescriptor& TabDescriptor);
};

UCLASS(Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class YQZY_API UYQZYTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FYQZYTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FYQZYTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FYQZYTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UYQZYTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FYQZYTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FYQZYTabDescriptor> PendingTabLabelInfoMap;
};
