...
#include "YQZYPlatformEmulationSettings.generated.h"
...
class UYQZYPlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UYQZYPlatformEmulationSettings();
...
private:
	...
	// The base device profile to pretend we are using when emulating device-specific device profiles applied from UYQZYSettingsLocal
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="YQZY.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="YQZY.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(InlineEditConditionToggle, ConsoleVariable="YQZY.Settings.ApplyDeviceProfilesInPIE"))
