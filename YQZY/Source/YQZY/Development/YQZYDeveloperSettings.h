...
#include "YQZYDeveloperSettings.generated.h"
...
class UYQZYDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UYQZYDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=YQZY, meta=(AllowedTypes="YQZYExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(BlueprintReadOnly, config, Category=YQZYBots)
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=YQZYBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=YQZYBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=YQZY)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in YQZY is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = YQZY, meta = (ConsoleVariable = "YQZYPC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=YQZY)
	TArray<FYQZYCheatToRun> CheatsToRun;
