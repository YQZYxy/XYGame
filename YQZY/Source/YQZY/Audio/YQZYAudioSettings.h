...
#include "YQZYAudioSettings.generated.h"
...
USTRUCT()
struct YQZY_API FYQZYSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "YQZYAudioSettings"))
class YQZY_API UYQZYAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FYQZYSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FYQZYSubmixEffectChainMap> LDRAudioSubmixEffectChain;
