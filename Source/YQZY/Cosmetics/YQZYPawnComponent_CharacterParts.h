...
#include "Cosmetics/YQZYCosmeticAnimationTypes.h"
#include "YQZYCharacterPartTypes.h"
#include "Net/Serialization/FastArraySerializer.h"

#include "YQZYPawnComponent_CharacterParts.generated.h"

class UYQZYPawnComponent_CharacterParts;
namespace EEndPlayReason { enum Type : int; }
struct FGameplayTag;
struct FYQZYCharacterPartList;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FYQZYSpawnedCharacterPartsChanged, UYQZYPawnComponent_CharacterParts*, ComponentWithChangedParts);

//////////////////////////////////////////////////////////////////////

// A single applied character part
USTRUCT()
struct FYQZYAppliedCharacterPartEntry : public FFastArraySerializerItem
{
	GENERATED_BODY()

	FYQZYAppliedCharacterPartEntry()
	{}

	FString GetDebugString() const;

private:
	friend FYQZYCharacterPartList;
	friend UYQZYPawnComponent_CharacterParts;
...
struct FYQZYCharacterPartList : public FFastArraySerializer
{
	GENERATED_BODY()

	FYQZYCharacterPartList()
		: OwnerComponent(nullptr)
	{
	}

	FYQZYCharacterPartList(UYQZYPawnComponent_CharacterParts* InOwnerComponent)
...
public:
	...
	{
		return FFastArraySerializer::FastArrayDeltaSerialize<FYQZYAppliedCharacterPartEntry, FYQZYCharacterPartList>(Entries, DeltaParms, *this);
	}

	FYQZYCharacterPartHandle AddEntry(FYQZYCharacterPart NewPart);
	void RemoveEntry(FYQZYCharacterPartHandle Handle);
...
private:
	...
	bool SpawnActorForEntry(FYQZYAppliedCharacterPartEntry& Entry);
	bool DestroyActorForEntry(FYQZYAppliedCharacterPartEntry& Entry);

private:
	// Replicated list of equipment entries
	UPROPERTY()
	TArray<FYQZYAppliedCharacterPartEntry> Entries;

	// The component that contains this list
	UPROPERTY(NotReplicated)
	TObjectPtr<UYQZYPawnComponent_CharacterParts> OwnerComponent;
...
template<>
struct TStructOpsTypeTraits<FYQZYCharacterPartList> : public TStructOpsTypeTraitsBase2<FYQZYCharacterPartList>
{
...
class UYQZYPawnComponent_CharacterParts : public UPawnComponent
{
	GENERATED_BODY()

public:
	UYQZYPawnComponent_CharacterParts(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	FYQZYCharacterPartHandle AddCharacterPart(const FYQZYCharacterPart& NewPart);
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Cosmetics)
	void RemoveCharacterPart(FYQZYCharacterPartHandle Handle);
...
public:
	...
	FYQZYSpawnedCharacterPartsChanged OnCharacterPartsChanged;

private:
	// List of character parts
	UPROPERTY(Replicated)
	FYQZYCharacterPartList CharacterPartList;

	// Rules for how to pick a body style mesh for animation to play on, based on character part cosmetics tags
	UPROPERTY(EditAnywhere, Category=Cosmetics)
	FYQZYAnimBodyStyleSelectionSet BodyMeshes;
};
