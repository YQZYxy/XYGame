...
#include "YQZYSettingsLocal.generated.h"
...
USTRUCT()
struct FYQZYScalabilitySnapshot
{
	GENERATED_BODY()

	FYQZYScalabilitySnapshot();
 ...
 */
UCLASS()
class UYQZYSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EYQZYStatDisplayMode GetPerfStatDisplayState(EYQZYDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EYQZYDisplayablePerformanceStat Stat, EYQZYStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UYQZYSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EYQZYDisplayablePerformanceStat, EYQZYStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FYQZYScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FYQZYScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UYQZYSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UYQZYSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	 ...
	 */
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UYQZYLocalPlayer* LocalPlayer);
	 ...
	 * @param LocalPlayer   local player to reset the keybinding on
	 */
	void ResetKeybindingToDefault(const FName MappingName, UYQZYLocalPlayer* LocalPlayer);

	/** Resets all keybindings to their default value in their input mapping context
	 * @param LocalPlayer   local player to reset the keybinding on
	 */
	void ResetKeybindingsToDefault(UYQZYLocalPlayer* LocalPlayer);
