...
#include "YQZYSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EYQZYAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UYQZYSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to YQZYPlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class UYQZYSettingsShared : public USaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UYQZYSettingsShared, FOnSettingChangedEvent, UYQZYSettingsShared* Settings);
...
public:
	...
	void Initialize(UYQZYLocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static UYQZYSettingsShared* LoadOrCreateSettings(const UYQZYLocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	EYQZYAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EYQZYAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EYQZYAllowBackgroundAudioSetting AllowAudioInBackground = EYQZYAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EYQZYGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EYQZYGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EYQZYGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EYQZYGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EYQZYGamepadSensitivity GamepadLookSensitivityPreset = EYQZYGamepadSensitivity::Normal;
	UPROPERTY()
	EYQZYGamepadSensitivity GamepadTargetingSensitivityPreset = EYQZYGamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UYQZYLocalPlayer> OwningPlayer = nullptr;
};
