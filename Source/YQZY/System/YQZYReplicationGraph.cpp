...
/**
*	
*	===================== YQZYReplicationGraph Replication =====================
*
*	Overview
*	
*		This changes the way actor relevancy works. AActor::IsNetRelevantFor is NOT used in this system!
*		
*		Instead, The UYQZYReplicationGraph contains UReplicationGraphNodes. These nodes are responsible for generating lists of actors to replicate for each connection.
...
*		
*		UYQZYReplicationGraph is intended to not be directly used by the game code. That is, you should not have to include YQZYReplicationGraph.h anywhere else.
*		Rather, UYQZYReplicationGraph depends on the game code and registers for events that the game code broadcasts (e.g., events for players joining/leaving teams).
*		This choice was made because it gives UYQZYReplicationGraph a complete holistic view of actor replication. Rather than exposing generic public functions that any
*		place in game code can invoke, all notifications are explicitly registered in UYQZYReplicationGraph::InitGlobalActorClassSettings.
*		
*	YQZY Nodes
*	
...
*		
*		UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection
...
*		
*		UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter
...
*		owning connection only) via UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection.
*		
...
*		
*	Dependent Actors (AYQZYWeapon)
*		
*		Replication Graph introduces a concept of dependent actor replication. This is an actor (AYQZYWeapon) that only replicates when another actor replicates (Pawn). I.e, the weapon
...
*		
*		See UYQZYReplicationGraph::OnCharacterWeaponChange: this is how actors are added/removed from the dependent actor list. 
*	
*	How To Use
*	
...
*		
*		Making something always relevant to connection: You will need to modify UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection. You will also want 
*		to make sure the actor does not get put in one of the other nodes. The safest way to do this is by setting its EClassRepNodeMapping to NotRouted in UYQZYReplicationGraph::InitGlobalActorClassSettings.
*
*	How To Debug
*	
...
*		
*		YQZY.RepGraph.PrintRouting - will print the EClassRepNodeMapping for each class. That is, how a given actor class is routed (or not) in the Replication Graph.
*	
*/

#include "YQZYReplicationGraph.h"
...
DEFINE_LOG_CATEGORY( YQZYLogRepGraph );

namespace YQZY::RepGraph
{
	float DestructionInfoMaxDist = 30000.f;
	static FAutoConsoleVariableRef CVarYQZYRepGraphDestructMaxDist(TEXT("YQZY.RepGraph.DestructInfo.MaxDist"), DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default);

	int32 DisplayClientLevelStreaming = 0;
	static FAutoConsoleVariableRef CVarYQZYRepGraphDisplayClientLevelStreaming(TEXT("YQZY.RepGraph.DisplayClientLevelStreaming"), DisplayClientLevelStreaming, TEXT(""), ECVF_Default);

	float CellSize = 10000.f;
	static FAutoConsoleVariableRef CVarYQZYRepGraphCellSize(TEXT("YQZY.RepGraph.CellSize"), CellSize, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarYQZYRepGraphSpatialBiasX(TEXT("YQZY.RepGraph.SpatialBiasX"), SpatialBiasX, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarYQZYRepSpatialBiasY(TEXT("YQZY.RepGraph.SpatialBiasY"), SpatialBiasY, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarYQZYRepDynamicActorFrequencyBuckets(TEXT("YQZY.RepGraph.DynamicActorFrequencyBuckets"), DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default);

	int32 DisableSpatialRebuilds = 1;
	static FAutoConsoleVariableRef CVarYQZYRepDisableSpatialRebuilds(TEXT("YQZY.RepGraph.DisableSpatialRebuilds"), DisableSpatialRebuilds, TEXT(""), ECVF_Default);

	int32 LogLazyInitClasses = 0;
	static FAutoConsoleVariableRef CVarYQZYRepLogLazyInitClasses(TEXT("YQZY.RepGraph.LogLazyInitClasses"), LogLazyInitClasses, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarYQZYRepTargetKBytesSecFastSharedPath(TEXT("YQZY.RepGraph.TargetKBytesSecFastSharedPath"), TargetKBytesSecFastSharedPath, TEXT(""), ECVF_Default);

	float FastSharedPathCullDistPct = 0.80f;
	static FAutoConsoleVariableRef CVarYQZYRepFastSharedPathCullDistPct(TEXT("YQZY.RepGraph.FastSharedPathCullDistPct"), FastSharedPathCullDistPct, TEXT(""), ECVF_Default);

	int32 EnableFastSharedPath = 1;
	static FAutoConsoleVariableRef CVarYQZYRepEnableFastSharedPath(TEXT("YQZY.RepGraph.EnableFastSharedPath"), EnableFastSharedPath, TEXT(""), ECVF_Default);
	...
	UReplicationDriver* ConditionalCreateReplicationDriver(UNetDriver* ForNetDriver, UWorld* World)
	{
		...
		{
			const UYQZYReplicationGraphSettings* YQZYRepGraphSettings = GetDefault<UYQZYReplicationGraphSettings>();

			// Enable/Disable via developer settings
			if (YQZYRepGraphSettings && YQZYRepGraphSettings->bDisableReplicationGraph)
			{
				UE_LOG(YQZYLogRepGraph, Display, TEXT("Replication graph is disabled via YQZYReplicationGraphSettings."));
				return nullptr;
			}

			UE_LOG(YQZYLogRepGraph, Display, TEXT("Replication graph is enabled for %s in world %s."), *GetNameSafe(ForNetDriver), *GetPathNameSafe(World));

			TSubclassOf<UYQZYReplicationGraph> GraphClass = YQZYRepGraphSettings->DefaultReplicationGraphClass.TryLoadClass<UYQZYReplicationGraph>();
			if (GraphClass.Get() == nullptr)
			{
				GraphClass = UYQZYReplicationGraph::StaticClass();
			}

			UYQZYReplicationGraph* YQZYReplicationGraph = NewObject<UYQZYReplicationGraph>(GetTransientPackage(), GraphClass.Get());
			return YQZYReplicationGraph;
		}
...
UYQZYReplicationGraph::UYQZYReplicationGraph()
{
	if (!UReplicationDriver::CreateReplicationDriverDelegate().IsBound())
	{
		UReplicationDriver::CreateReplicationDriverDelegate().BindLambda(
			...
			{
				return YQZY::RepGraph::ConditionalCreateReplicationDriver(ForNetDriver, World);
			});
	}
}

void UYQZYReplicationGraph::ResetGameWorldState()
{
	...
	for (UNetReplicationGraphConnection* ConnManager : Connections)
	{
		...
		{
			if (UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = Cast<UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection>(ConnectionNode))
			{
...
EClassRepNodeMapping UYQZYReplicationGraph::GetClassNodeMapping(UClass* Class) const
{
	if (!Class)
	{
...
void UYQZYReplicationGraph::RegisterClassRepNodeMapping(UClass* Class)
{
	EClassRepNodeMapping Mapping = GetClassNodeMapping(Class);
	ClassRepNodePolicies.Set(Class, Mapping);
}

void UYQZYReplicationGraph::InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool Spatialize) const
{
	AActor* CDO = Class->GetDefaultObject<AActor>();
	if (Spatialize)
	{
		Info.SetCullDistanceSquared(CDO->NetCullDistanceSquared);
		UE_LOG(YQZYLogRepGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(YQZYLogRepGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->NetUpdateFrequency);
}

bool UYQZYReplicationGraph::ConditionalInitClassReplicationInfo(UClass* ReplicatedClass, FClassReplicationInfo& ClassInfo)
{
...
void UYQZYReplicationGraph::AddClassRepInfo(UClass* Class, EClassRepNodeMapping Mapping)
{
	if (IsSpatialized(Mapping))
	{
		if (Class->GetDefaultObject<AActor>()->bAlwaysRelevant)
		{
			UE_LOG(YQZYLogRepGraph, Warning, TEXT("Replicated Class %s is AlwaysRelevant but is initialized into a spatialized node (%s)"), *Class->GetName(), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
		}
	}
...
void UYQZYReplicationGraph::RegisterClassReplicationInfo(UClass* ReplicatedClass)
{
	...
	if (ConditionalInitClassReplicationInfo(ReplicatedClass, ClassInfo))
	{
		...
		UE_LOG(YQZYLogRepGraph, Log, TEXT("Setting %s - %.2f"), *GetNameSafe(ReplicatedClass), ClassInfo.GetCullDistance());
	}
}

void UYQZYReplicationGraph::InitGlobalActorClassSettings()
{
	...
	GlobalActorReplicationInfoMap.SetInitClassInfoFunc(
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (YQZY::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					EClassRepNodeMapping Mapping = ClassRepNodePolicies.GetChecked(Class);
					UE_LOG(YQZYLogRepGraph, Warning, TEXT("%s was Lazy Initialized. (Parent: %s) %d."), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()), (int32)Mapping);
					...
					{
						UE_LOG(YQZYLogRepGraph, Warning, TEXT("Differences Found!"));
						FString DebugStr = ParentRepInfo.BuildDebugStringDelta();
						UE_LOG(YQZYLogRepGraph, Warning, TEXT("  Parent: %s"), *DebugStr);

						DebugStr = ClassInfo.BuildDebugStringDelta();
						UE_LOG(YQZYLogRepGraph, Warning, TEXT("  Class : %s"), *DebugStr);
					}
				}
				else
				{
					UE_LOG(YQZYLogRepGraph, Warning, TEXT("%s skipped Lazy Initialization because it does not differ from its parent. (Parent: %s)"), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()));
	...
	const UYQZYReplicationGraphSettings* YQZYRepGraphSettings = GetDefault<UYQZYReplicationGraphSettings>();
	check(YQZYRepGraphSettings);

	// Set Classes Node Mappings
	for (const FRepGraphActorClassSettings& ActorClassSettings : YQZYRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddClassRepInfoToMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(YQZYLogRepGraph, Log, TEXT("ActorClassSettings -- AddClassRepInfo - %s :: %i"), *StaticActorClass->GetName(), int(ActorClassSettings.ClassNodeMapping));
...
#if WITH_GAMEPLAY_DEBUGGER
	AddClassRepInfo(AGameplayDebuggerCategoryReplicator::StaticClass(), EClassRepNodeMapping::NotRouted);				// Replicated via UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	CharacterClassRepInfo.SetCullDistanceSquared(AYQZYCharacter::StaticClass()->GetDefaultObject<AYQZYCharacter>()->NetCullDistanceSquared);
...

	{
		...
		FRepMovement DefaultRepMovement = AYQZYCharacter::StaticClass()->GetDefaultObject<AYQZYCharacter>()->GetReplicatedMovement(); // Use the same quantization settings as our default replicatedmovement
	...
	{
		bool bSuccess = false;
		if (AYQZYCharacter* Character = Cast<AYQZYCharacter>(Actor))
		{
	...
	FastSharedPathConstants.MaxBitsPerFrame = (int32)((float)(YQZY::RepGraph::TargetKBytesSecFastSharedPath * 1024 * 8) / NetDriver->GetNetServerMaxTickRate());
	FastSharedPathConstants.DistanceRequirementPct = YQZY::RepGraph::FastSharedPathCullDistPct;

	SetClassInfo(AYQZYCharacter::StaticClass(), CharacterClassRepInfo);

	// ---------------------------------------------------------------------
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.ListSize = 12;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.NumBuckets = YQZY::RepGraph::DynamicActorFrequencyBuckets;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.BucketThresholds.Reset();
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.EnableFastPath = (YQZY::RepGraph::EnableFastSharedPath > 0);
	...
	UE_LOG(YQZYLogRepGraph, Log, TEXT(""));
	UE_LOG(YQZYLogRepGraph, Log, TEXT("Class Routing Map: "));
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		UE_LOG(YQZYLogRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
	}

	UE_LOG(YQZYLogRepGraph, Log, TEXT(""));
	UE_LOG(YQZYLogRepGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(YQZYLogRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = YQZY::RepGraph::DestructionInfoMaxDist * YQZY::RepGraph::DestructionInfoMaxDist;
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : YQZYRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddToRPC_Multicast_OpenChannelForClassMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(YQZYLogRepGraph, Log, TEXT("ActorClassSettings -- RPC_Multicast_OpenChannelForClass - %s"), *StaticActorClass->GetName());
...
void UYQZYReplicationGraph::InitGlobalGraphNodes()
{
	// -----------------------------------------------
	//	Spatial Actors
	// -----------------------------------------------
	...
	GridNode->CellSize = YQZY::RepGraph::CellSize;
	GridNode->SpatialBias = FVector2D(YQZY::RepGraph::SpatialBiasX, YQZY::RepGraph::SpatialBiasY);

	if (YQZY::RepGraph::DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void UYQZYReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping UYQZYReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
void UYQZYReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UYQZYReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
	switch(Policy)
	{
		...
		case EClassRepNodeMapping::RelevantAllConnections:
		{
			...
			else
			{
				...
				{
					UE_LOG(YQZYLogRepGraph, Warning, TEXT("Actor %s was not found in AlwaysRelevantStreamingLevelActors list. LevelName: %s"), *GetActorRepListTypeDebugString(ActorInfo.Actor), *ActorInfo.StreamingLevelName.ToString());
				}				
			}
...
#if WITH_GAMEPLAY_DEBUGGER
void UYQZYReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	CHECK_WORLDS(Debugger);

	auto GetAlwaysRelevantForConnectionNode = [this](APlayerController* Controller) -> UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection*
	{
		if (Controller)
		{
	...
	if (UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(OldOwner))
	{
		AlwaysRelevantConnectionNode->GameplayDebugger = nullptr;
	}

	if (UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	ReplicationActorList.Reset();
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	UYQZYReplicationGraph* YQZYGraph = CastChecked<UYQZYReplicationGraph>(GetOuter());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AYQZYPlayerController* PC = Cast<AYQZYPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (AYQZYCharacter* Pawn = Cast<AYQZYCharacter>(PC->GetPawn()))
			{
			...
			if (AYQZYCharacter* ViewTargetPawn = Cast<AYQZYCharacter>(CurViewer.ViewTarget))
			{
	...
	TMap<FName, FActorRepListRefView>& AlwaysRelevantStreamingLevelActors = YQZYGraph->AlwaysRelevantStreamingLevelActors;
	...
	for (int32 Idx=AlwaysRelevantStreamingLevelsNeedingReplication.Num()-1; Idx >= 0; --Idx)
	{
		...
		if (Ptr == nullptr)
		{
			...
			UE_CLOG(YQZY::RepGraph::DisplayClientLevelStreaming > 0, YQZYLogRepGraph, Display, TEXT("CLIENTSTREAMING Removing %s from AlwaysRelevantStreamingLevelActors because FActorRepListRefView is null. %s "), *StreamingLevel.ToString(),  *Params.ConnectionManager.GetName());
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(YQZY::RepGraph::DisplayClientLevelStreaming > 0, YQZYLogRepGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, 1, false);
			}
			else
			{
				UE_CLOG(YQZY::RepGraph::DisplayClientLevelStreaming > 0, YQZYLogRepGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(YQZYLogRepGraph, Warning, TEXT("UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(YQZY::RepGraph::DisplayClientLevelStreaming > 0, YQZYLogRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(YQZY::RepGraph::DisplayClientLevelStreaming > 0, YQZYLogRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void UYQZYReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		UYQZYReplicationGraph* YQZYGraph = CastChecked<UYQZYReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = YQZYGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter::UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
...
void UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...
void UYQZYReplicationGraphNode_PlayerStateFrequencyLimiter::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	DebugInfo.Log(NodeName);
	DebugInfo.PushIndent();	
...
void UYQZYReplicationGraph::PrintRepNodePolicies()
{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("YQZY Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
...
FAutoConsoleCommandWithWorldAndArgs YQZYPrintRepNodePoliciesCmd(TEXT("YQZY.RepGraph.PrintRouting"),TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (TObjectIterator<UYQZYReplicationGraph> It; It; ++It)
		{
...
FAutoConsoleCommandWithWorldAndArgs ChangeFrequencyBucketsCmd(TEXT("YQZY.RepGraph.FrequencyBuckets"), TEXT("Resets frequency bucket count."), FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray< FString >& Args, UWorld* World) 
{
	int32 Buckets = 1;
	if (Args.Num() > 0)
	{
	...
	UE_LOG(YQZYLogRepGraph, Display, TEXT("Setting Frequency Buckets to %d"), Buckets);
