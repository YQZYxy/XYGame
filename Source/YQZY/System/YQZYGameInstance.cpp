...
#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYGameInstance)

namespace YQZY
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarYQZYTestEncryption(
		TEXT("YQZY.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarYQZYUseDTLSEncryption(
		TEXT("YQZY.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using YQZY.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarYQZYTestDTLSFingerprint(
		TEXT("YQZY.TestDTLSFingerprint"),
...
UYQZYGameInstance::UYQZYGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UYQZYGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(YQZYGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(YQZYGameplayTags::InitState_DataAvailable, false, YQZYGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(YQZYGameplayTags::InitState_DataInitialized, false, YQZYGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(YQZYGameplayTags::InitState_GameplayReady, false, YQZYGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UYQZYGameInstance::OnPreClientTravelToSession);
	}
}

void UYQZYGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AYQZYPlayerController* UYQZYGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AYQZYPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UYQZYGameInstance::CanJoinRequestedSession() const
{
...
void UYQZYGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (YQZY::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (YQZY::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("YQZYTest.pem");
...
void UYQZYGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UYQZYGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (YQZY::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (YQZY::bUseDTLSEncryption)
		{
