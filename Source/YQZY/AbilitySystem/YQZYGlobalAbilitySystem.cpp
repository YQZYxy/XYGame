...
#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYGlobalAbilitySystem)

void FGlobalAppliedAbilityList::AddToASC(TSubclassOf<UGameplayAbility> Ability, UYQZYAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(UYQZYAbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, UYQZYAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(UYQZYAbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
UYQZYGlobalAbilitySystem::UYQZYGlobalAbilitySystem()
{
}

void UYQZYGlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);		
		for (UYQZYAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UYQZYGlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void UYQZYGlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void UYQZYGlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void UYQZYGlobalAbilitySystem::RegisterASC(UYQZYAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UYQZYGlobalAbilitySystem::UnregisterASC(UYQZYAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
