...
#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UYQZYCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UYQZYDamageExecution::UYQZYDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UYQZYDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FYQZYGameplayEffectContext* TypedContext = FYQZYGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UYQZYTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UYQZYTeamSubsystem>();
	...
	if (const IYQZYAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UYQZYHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
