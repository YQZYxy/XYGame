...
#include "YQZYAudioSettings.h"
#include "Settings/YQZYSettingsLocal.h"
#include "Sound/SoundEffectSubmix.h"
#include "SoundControlBus.h"
#include "SoundControlBusMix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void UYQZYAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void UYQZYAudioMixEffectsSubsystem::Deinitialize()
{
...
bool UYQZYAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UYQZYAudioMixEffectsSubsystem::PostInitialize()
{
	if (const UYQZYAudioSettings* YQZYAudioSettings = GetDefault<UYQZYAudioSettings>())
	{
		if (UObject* ObjPath = YQZYAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from YQZY Audio Settings."));
			}
		}

		if (UObject* ObjPath = YQZYAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from YQZY Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = YQZYAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from YQZY Audio Settings."));
			}
		}
		...
		for (const FYQZYSubmixEffectChainMap& SoftSubmixEffectChain : YQZYAudioSettings->HDRAudioSubmixEffectChain)
		{
			FYQZYAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FYQZYSubmixEffectChainMap& SoftSubmixEffectChain : YQZYAudioSettings->LDRAudioSubmixEffectChain)
		{
			FYQZYAudioSubmixEffectsChain NewEffectChain;
...
void UYQZYAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UYQZYSettingsLocal* YQZYSettingsLocal = GetDefault<UYQZYSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, YQZYSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, YQZYSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, YQZYSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, YQZYSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, YQZYSettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(YQZYSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void UYQZYAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FYQZYAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FYQZYAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FYQZYAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FYQZYAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UYQZYAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UYQZYAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UYQZYAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
