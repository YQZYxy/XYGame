...
#include "YQZYTeamSubsystem.generated.h"

class AActor;
class AYQZYPlayerState;
class AYQZYTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnYQZYTeamDisplayAssetChangedDelegate, const UYQZYTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FYQZYTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<AYQZYTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<AYQZYTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UYQZYTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnYQZYTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(AYQZYTeamInfoBase* Info);
	void RemoveTeamInfo(AYQZYTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class EYQZYTeamComparison : uint8
{
...
UCLASS()
class YQZY_API UYQZYTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(AYQZYTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(AYQZYTeamInfoBase* TeamInfo);
	...
	const AYQZYPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	EYQZYTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	EYQZYTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UYQZYTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UYQZYTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UYQZYTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnYQZYTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FYQZYTeamTrackingInfo> TeamMap;
