...
#include "YQZYGameplayAbility_RangedWeapon.h"
#include "Weapons/YQZYRangedWeaponInstance.h"
#include "Physics/YQZYCollisionChannels.h"
#include "YQZYLog.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/YQZYWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/YQZYGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(YQZYGameplayAbility_RangedWeapon)
...
namespace YQZYConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("YQZY.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("YQZY.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("YQZY.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UYQZYGameplayAbility_RangedWeapon::UYQZYGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UYQZYRangedWeaponInstance* UYQZYGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UYQZYRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UYQZYGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(YQZYLog, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UYQZYRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UYQZYGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UYQZYGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UYQZYGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return YQZY_TraceChannel_Weapon;
}

FHitResult UYQZYGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UYQZYGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UYQZYGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EYQZYAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UYQZYWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UYQZYWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EYQZYAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EYQZYAbilityTargetingSource::CameraTowardsFocus) || (Source == EYQZYAbilityTargetingSource::PawnTowardsFocus) || (Source == EYQZYAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EYQZYAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EYQZYAbilityTargetingSource::WeaponForward) || (Source == EYQZYAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EYQZYAbilityTargetingSource::PawnTowardsFocus) || (Source == EYQZYAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UYQZYGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (YQZYConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, YQZYConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UYQZYGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UYQZYRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EYQZYAbilityTargetingSource::CameraTowardsFocus);
		...
		if (YQZYConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, YQZYConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UYQZYGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UYQZYRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (YQZYConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, YQZYConsoleVariables::DrawBulletHitRadius, FColor::Red, false, YQZYConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UYQZYGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UYQZYGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UYQZYGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UYQZYWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UYQZYWeaponStateComponent>())
					{
		...
		{
			UE_LOG(YQZYLog, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UYQZYGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UYQZYWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UYQZYWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FYQZYGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FYQZYGameplayAbilityTargetData_SingleTargetHit();
