...
public class YQZYTarget : TargetRules
{
    public YQZYTarget(TargetInfo Target) : base(Target)
    {
    ...
    internal static void ApplySharedYQZYTargetSettings(TargetRules Target)
    {
        ...
        if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
        {
            ...
            YQZYTarget.ConfigureGameFeaturePlugins(Target);
        }
        else
        {
            // !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
            ...
            else
            {
                ...
                if (!bHasWarnedAboutShared)
                {
                    ...
                    Logger.LogWarning("YQZYEOS and dynamic target options are disabled when packaging from an installed version of the engine");
                }
            }
        }
    }
