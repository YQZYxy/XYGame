...
#include "Private/AssetTypeActions_YQZYContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
{
	return HasNoPlayWorld() && !GetDefault<UYQZYDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UYQZYDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FYQZYEditorModule
 */
class FYQZYEditorModule : public FDefaultGameModuleImpl
{
	typedef FYQZYEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FYQZYEditorModule::ModulesChangedCallback);
		...
		AssetTools.RegisterAssetTypeActions(MakeShareable(new FAssetTypeActions_YQZYContextEffectsLibrary));
	}

	void OnBeginPIE(bool bIsSimulating)
	{
		UYQZYExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UYQZYExperienceManager>();
...
IMPLEMENT_MODULE(FYQZYEditorModule, YQZYEditor);
